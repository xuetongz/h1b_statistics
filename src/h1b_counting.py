...
class H1B:
    ...
    def read_file(self,status,field_list):
        '''read file, find position of needed field,
           then store wanted information in a nested dictionary
           input: status--status of instance, e.g "CERTIFIED"
                  field_lsit--list of field you are interested in, e.g ['SOC_NAME', 'WORKSITE_STATE']
    ...
    def sort_wanted_fields(self):
        ''' sort nested dictionary by value,when there is a tie, sort alphabetically
        '''
    ...
    def get_top_n(self, row_name, top_n):
        '''
        from a nested dictionay,return top key value pair from a nested dictionary, stored as tuple
        input:row_name -- e.g WORKSITE_STATE, SOC_NAME
              top_n -- number of top items you want to get
        '''
        return self.wanted_fields[row_name][:top_n]

    def write_file(self, row_name, wanted_name, top_n, path):
        '''
        write tuple in to csv file
        input: row_name -- name of field stored as key of the first layer of the dictionay
               wanted_name -- name of file after 'top'
               top_n --top number you want to know
